...
- 最上層使用 tab 分成 `General`、`AI Provider`、`Embedding`、`Notifications`、`Starter`、`Agent Flow`、`Tools`。
- `Starter` 與 `Agent Flow` 雖然是 library 形態，但在現行版本中仍是 settings 內的完整分頁，不是獨立 URL。
...
|- Main panel
   ...
   |  |- Ollama / LM Studio / Gemini / Azure OpenAI embedding cards
...
- default embedding provider
- Ollama embedding URL / model
- LM Studio embedding URL / model / API key
- Gemini embedding model / API key
- Azure OpenAI embedding endpoint / deployment / API version / API key
- Ollama embedding model refresh

這個 tab 主要服務 Knowledge Base QA Tester 與 retrieval 類工具。它和聊天 provider 分開，因為回答模型與向量模型常常不是同一個。
...
### 8. Starter / Agent Flow Tabs

- `Starter` tab 直接管理 built-in + custom starters
- `Agent Flow` tab 直接管理 built-in flow templates + saved flows
- `Agent Flow` tab 另外提供 Batch URL QA logs 入口
...
工具頁要用 extension URL 開啟，避免 `file://` 限制，並能重用 Open Copilot 已儲存的 provider / embedding 設定。
...
|                                                                                |
| [General] [AI Provider] [Embedding] [Notifications] [Starter] [Agent Flow]    |
| [Tools]                                                                        |
|                                                                                |
| General                                                                        |
...
|                                                                                |
| Embedding                                                                      |
| Default Embedding Provider [ollama v]                                          |
| Ollama / LM Studio / Gemini / Azure OpenAI embedding settings                 |
|                                                                                |
| Notifications                                                                  |
...
|                                                                                |
| Tools                                                                          |
| [JSONL QA Editor] [Knowledge Base QA Tester]                                  |
+--------------------------------------------------------------------------------+
```
...
- 這是 settings 內的 extension utility launcher。
- 工具頁必須從 extension origin 開啟，不以本機 `file://` 作為主要使用方式。
- 目前至少包含 JSONL QA Editor 與 Knowledge Base QA Tester。

### Tool card 規則

- 每個工具用一張 card 顯示名稱、用途、開啟按鈕與簡短使用情境。
- `JSONL QA Editor` 用於編輯 FAQ-style JSONL record、AI assist、暫存與寫回。
- `Knowledge Base QA Tester` 用於驗證 chunking、embedding retrieval、top-k grounding、batch QA 與 judge/advisor。
...
- Starter / Agent Flow tab 內的卡片 grid 必須保留足夠間距，避免資訊擠壓

## DOM and Section Contract

- top-level settings tabs 必須存在
- provider tab 與 top-level settings tabs 是兩層不同導覽，不可合併
- `Embedding` 必須是 top-level tab，不可藏在 AI Provider card 裡
- `Tools` 必須是 top-level tab，用卡片開啟工具頁
- `Starter` / `Agent Flow` 必須以 settings 內完整 tab 形式存在
- `Starter` / `Agent Flow` 都必須使用卡片視圖作為主視圖
- `Starter` tab 要保留 metrics 區塊：
  - total count
  - built-in count
  - custom count
- `Agent Flow` tab 要保留 metrics 區塊：
  - stored count
  - linked skills
...
flowchart TD
  ...
  D -- Switch tab --> G[Open General / Provider / Embedding / Notifications / Starter / Flow / Tools tab]
  D -- Folder/Drive --> H[Pick/connect and sync]
  D -- Open tool --> I[Open extension utility page]
...
- Embedding 與 Tools 必須可從 top-level tabs 進入
